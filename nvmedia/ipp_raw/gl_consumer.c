...
// Lookup table width and height
// These have to be the same as the LAC1 statistics
// horizontal and vertical number of windows
#define LOOKUP_WIDTH    (1280 / 32)
#define LOOKUP_HEIGHT   (1080 / 32)
...
static const char fshader[] ={
    ...
    "    lookup_coord.xy = TexCoord.xy;\n"
    "    lookup_coord.z = color.r;\n"
    "    ltm = texture(Lookup, lookup_coord).xw;\n"
    "    color.r = ltm.x;\n"
    "    color.g = ((color.g - 0.5) * ltm.y * 4.0) + 0.5;\n"
    "    color.b = ((color.b - 0.5) * ltm.y * 4.0) + 0.5;\n"
    "    color.rgb += colorOff;\n"
    "    color.rgb *= colorMat;\n"
...
static void initShader(GLConsumerCtx *ctx)
{
    ...
    lookupLoc = glGetUniformLocation(shaderID, "Lookup");
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, GL_LUMINANCE_ALPHA,
                LOOKUP_WIDTH, LOOKUP_HEIGHT, NVLTM_DEFAULT_LOOKUP_DEPTH, 0,
                GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, ctx->lookupTable);
...
void *GlConsumerProc(void *data)
{
    ...
    void *ltmData = NULL;
    NvS32 ltmDatasize = 0;
    NvLTMStatus ltmStatus;
    // The dimension of the 3D texture
    NvLTMTableDimension texDim;
    NvU32 i = 0;
    EglUtilState *eglUtil = NULL;
    EglStreamClient *eglStrmCtx = NULL;
    ...
    while(!(*ctx->quit)) {
        ...
        for(i = 0; i < ctx->ippNum; i++) {
            ...
            if(!eglQueryStreamMetadataNV(
                        eglUtil->display,                // display
                        eglStrmCtx->eglStream[i],             // stream
                        EGL_CONSUMER_METADATA_NV,      // name
                        0,                             // block id
                        0,                             // offset
                        4,                             // size
                        &ltmDatasize)) {                  // data
                LOG_ERR("GL consumer: eglQueryStreamKHR EGL_CONSUMER_METADATA_NV failed\n");
            }
            ...
            if(ltmDatasize) {
                ...
                LOG_INFO("%s:ltmData %p, ltmDataSize %d\n", __func__, ltmData, ltmDatasize);
            }

            ltmStatus = NvLTMGetLookupTableDimension(
                            ctx->nvLTMHandle,
                            ltmData,
                            &texDim);

            if(ltmStatus != NVLTM_STATUS_OK) {
                LOG_ERR("%s: NvLTMGetLookupTableDimension() failed\n", __func__);
                goto done;
            }

            // Generate Look up Table
            ltmStatus = NvLTMProcessToneMap(
                                    ctx->nvLTMHandle,
                                    ltmData,
                                    ctx->lookupTable);

            if(ltmStatus != NVLTM_STATUS_OK) {
                LOG_ERR("Error processing lookup table\n");
                goto done;
            }
            ...
            glBindTexture(GL_TEXTURE_3D, lookup_tex);
            glTexImage3D(GL_TEXTURE_3D, 0, GL_LUMINANCE_ALPHA,
                texDim.width, texDim.height, texDim.depth, 0,
                GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, ctx->lookupTable);
                             ...
                             InteropContext *interopCtx)
{
    ...
    // Generate Meta Data Buffer
        // Generate lookup table
    NvLTMConfiguration ltmConfig = {
        .lookupWidth = LOOKUP_WIDTH,
        .lookupHeight = LOOKUP_HEIGHT,
        .lookupDepth = NVLTM_MAX_LOOKUP_DEPTH
    };

    ltmStatus = NvLTMCreate(&ltmConfig, &ctx->nvLTMHandle);
    if(ltmStatus != NVLTM_STATUS_OK) {
        LOG_ERR("Error creating LTM handle\n");
        goto failed;
    }

    ltmStatus = NvLTMGetLookupTableSize(
                                ctx->nvLTMHandle,
                                &lookupTableSize);
    if(ltmStatus != NVLTM_STATUS_OK) {
        LOG_ERR("Error getting max lookup table size\n");
        goto failed;
    }
    LOG_INFO("Lookup table size: %d\n", lookupTableSize);

    ctx->ltmData = calloc(1, MAX_METASIZE);
    if(!ctx->ltmData) {
        LOG_ERR("Error allocating memroy for ltmdata \n");
        goto failed;
    }

    ctx->lookupTable = calloc(1, lookupTableSize);
    if(!ctx->lookupTable) {
        LOG_ERR("Error allocating lookup table\n");
        goto failed;
    }

    NvThreadCreate(&ctx->glConsumerThread, (void *)GlConsumerProc,
                   (void *)ctx, NV_THREAD_PRIORITY_NORMAL);
    ...
    return ctx;
failed:
    if(IsFailed(GlConsumerFini(ctx))) {
...
NvMediaStatus GlConsumerFini(GLConsumerCtx *ctx)
{
    ...
    if(ctx->glConsumerThread) {
        status = NvThreadDestroy(ctx->glConsumerThread);
        if(status != NVMEDIA_STATUS_OK)
            LOG_ERR("%s: Failed to destroy glConsumerThread \n", __func__);
    }

    //destroy LTM handle
    NvLTMDestroy(ctx->nvLTMHandle);
    //release ltmdata buffer
    free(ctx->ltmData);
    //release lookup buffer
    free(ctx->lookupTable);
    free(ctx);
    return status;
}
