        ...
        for(i = 1; i < argc; i++) {
            ...
            if(strcasecmp(&argv[i][1], "h") == 0) {
               ...
               if(args->pluginFlag == NVMEDIA_NOACPLUGIN)
                   ...
                   LOG_MSG("WARN: more than one plugin type is detected, last one is effective\n");
           } else if(!strcasecmp(argv[i], "--nvbracketae")) {
               if(args->pluginFlag == NVMEDIA_NOACPLUGIN)
                   args->pluginFlag = NVMEDIA_NVBEPLUGIN;
               else
                   LOG_MSG("WARN: more than one plugin type is detected, last one is effective\n");
...
PrintUsage ()
{
    ...
    LOG_MSG("Usage: nvmipp_raw -cf [file]  -c [name] -d [n] --aggregate [n] --[plugintype]\n");
    ...
    LOG_MSG("--nvbracketae     Use Nvmedia IPP bracketed auto-exposure plugin for control algorithm components\n");
