...
)
{
    ...
    if(device->iscBroadcastSerializer){
        // Set input mode first in case BWS = 1
    ...
    for(i = 0; i < configParam->sensorsNum; i++) {
        if(device->iscSensor[i]) {
            ...
            status = NvMediaISCWriteParameters(device->iscSensor[i],
                                               ...
                                               &paramOV10640);
            if(status != NVMEDIA_STATUS_OK) {
                LOG_ERR("%s: Failed to set short channel pre-matix\n", __func__);
                return status;
            }
        }
    }
...
)
{
    ...
    status = ImgDevSetProperty(GetDriver_ref_max9286_9271_ov10640(),
                                        configParam,
                                        device);
    if(status != NVMEDIA_STATUS_OK) {
...
static ImgProperty properties[] = {
    ...
    IMG_PROPERTY_ENTRY(1280x1080,     24,  30, 50002260,     2,         2,       raw12,       bggr),
    IMG_PROPERTY_ENTRY(1280x800,      24,  30, 36005340,     2,         2,       raw12,       bggr),
};


static ImgDevDriver device = {
    .name = "ref_max9286_9271_ov10640",
    .Init = Init,
    .Deinit = Deinit,
    .Start = Start,
...
ImgDevDriver *
GetDriver_ref_max9286_9271_ov10640(void)
{
    return &device;
}
