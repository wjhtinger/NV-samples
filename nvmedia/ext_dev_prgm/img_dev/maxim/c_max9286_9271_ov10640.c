...
Init(ExtImgDevParam *configParam)
{
    ...
    status = ImgDevSetProperty(GetDriver_c_max9286_9271_ov10640(),
                                        configParam,
                                        device);
    if(status != NVMEDIA_STATUS_OK) {
        ...
        for(i = 0; i < configParam->sensorsNum; i++) {
            if(device->iscSensor[i]) {
                ...
                status = NvMediaISCWriteParameters(device->iscSensor[i],
                                ...
                                &paramOV10640);
                if(status != NVMEDIA_STATUS_OK) {
                    LOG_ERR("%s: Failed to set sensor config\n", __func__);
                    goto failed;
                }
            }
        }
...
ImgDevDriver *
GetDriver_c_max9286_9271_ov10640(void)
{
    return &device;
}
