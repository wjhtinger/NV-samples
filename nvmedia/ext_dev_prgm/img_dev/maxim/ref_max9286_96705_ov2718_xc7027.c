            ...
            if(configParam->serAddr[i] &&
                ...
                if(status != NVMEDIA_STATUS_OK) {
                    LOG_ERR("%s: Failed to set serializer device %d address\n", __func__,
                            configParam->serAddr[remapIdx[i]]);
                    return status;
                }
    ...
    ExtImgDevParam *configParam)
{
    ...
    status = ImgDevSetProperty(GetDriver_ref_max9286_96705_ov2718_xc7027(),
                                        configParam,
                                        device);
    if(status != NVMEDIA_STATUS_OK) {
...
static ImgDevDriver device = {
    .name = "ref_max9286_96705_ov2718_xc7027",
    .Init = Init,
    .Deinit = Deinit,
    .Start = Start,
...
ImgDevDriver *
GetDriver_ref_max9286_96705_ov2718_xc7027(void)
{
    return &device;
}
