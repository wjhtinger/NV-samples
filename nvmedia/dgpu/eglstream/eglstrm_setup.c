...
EGLStreamSingleProcInit(EGLDisplay display,
                        ...
                        NvBool consumerondGPU)
{
    EglStreamClient *client = NULL;

    client = calloc(1, sizeof(EglStreamClient));
    if (!client) {
    ...
    if(consumerondGPU){
        ...
        if(fileDescriptor == EGL_NO_FILE_DESCRIPTOR_KHR) {
            ...
            eglDestroyStreamKHR(display, client->eglStream);
            goto fail;
        }

        client->eglStream_dGPU = eglCreateStreamFromFileDescriptorKHR(display_dGPU,
                                                fileDescriptor);
        if (client->eglStream_dGPU == EGL_NO_STREAM_KHR) {
...
void EGLStreamFini(EglStreamClient *client, NvBool consumerondGPU)
{
    if (client) {
        ...
        if(consumerondGPU && client->eglStream_dGPU) {
            if(!eglDestroyStreamKHR(client->display_dGPU, client->eglStream_dGPU)) {
