...
#define GET_FRAME_TIMEOUT (200)

NvMediaBool                 Producerstopped = NVMEDIA_FALSE;
...
IPPCreateICPComponent(IPPCtx *ctx)
{
    ...
    if(!ctx->useVirtualChannels) {
        ...
        icpSettingsEx.interfaceType  = ctx->icpSettings.interfaceType;
        icpSettingsEx.interfaceLanes = ctx->icpSettings.interfaceLanes;
        icpSettingsEx.numVirtualChannels = ctx->imagesNum;

        for (i = 0; i < ctx->imagesNum; i++) {
            icpSettingsEx.settings[i].virtualChannelIndex = i;
            memcpy(&icpSettingsEx.settings[i].icpSettings, &ctx->icpSettings,
                   sizeof(NvMediaICPSettings));
        }

        ctx->icpComponent = NvMediaIPPComponentCreate(ctx->ippPipeline[0],
                                                      NVMEDIA_IPP_COMPONENT_CAPTURE_EX,
                                                      bufferPools,
                                                      &icpSettingsEx);
    }
    if (!ctx->icpComponent) {
...
GetOutputThreadFunc(void *data)
{
    ...
    while(!ctx->quit) {
        ...
        retImage = NULL;
        status = InteroppostSurface(ctx->interopCtx, output.image, &retImage, pipelineNum);

        //If postsurface failed, return the current surface back to IPP
        if(IsFailed(status)){
            retImage = output.image;
        }
        if(retImage){
   ...
   ctx->ippProcThrdexited[pipelineNum] = NVMEDIA_TRUE;
    return status;
}
...
IPPInit(IPPCtx *ctx, TestArgs *args)
{
    ...
    for (i = 0; i < ctx->ippPipelineNum; i++) {
        ...
        ctx->ippProcThrdexited[i] = NVMEDIA_FALSE;
...
IPPStop (IPPCtx *ctx)
{
    NvMediaBool ippstopped;
    NvU32 i;

    do {
        usleep(10000);
        ippstopped = NVMEDIA_TRUE;
        for(i = 0; i < ctx->ippPipelineNum; i++) {
            ippstopped &= ctx->ippProcThrdexited[i];
        }
        LOG_DBG("IPPStop: ippstopped is %d \n", ippstopped);
    }while(!ippstopped);

    Producerstopped = NVMEDIA_TRUE;

    for(i = 0; i < ctx->ippPipelineNum; i++) {
        if (IsFailed(NvMediaIPPPipelineStop(ctx->ippPipeline[i]))) {
