...
static void initShader(GLConsumer *ctx)
{
    ...
    eglStreamConsumerGLTextureExternalKHR(ctx->display, ctx->eglStream);
    // TODO: check result
...
NvMediaStatus glConsumerInit_rgb(volatile NvBool *consumerDone, GLConsumer *ctx, EGLDisplay eglDisplay, EGLStreamKHR eglStream,EglUtilState* state,TestArgs *args)
{
...
void glConsumerCleanup_rgb(GLConsumer *ctx)
{
