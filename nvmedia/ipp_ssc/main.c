...
main(int argc, char **argv)
{
    ...
    if (IsFailed(DisplayInit(ctx))) {
        LOG_ERR("%s: Error in display initialization\n", __func__);
        goto failed;
    }

    // initialize image writing
    if (IsFailed(WriteImageInit(ctx))) {
        LOG_ERR("%s: Error in image writing initialization\n", __func__);
        goto failed;
    }
