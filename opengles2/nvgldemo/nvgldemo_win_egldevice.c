...
static bool NvGlDemoCreateEglDevice(EGLint devIndx)
{
    ...
    // Obtain layers and allocate an array of window pointers for them
    ...
    if (devOut->layerList && devOut->windowList) {
        ...
        peglGetOutputLayersEXT(devOut->eglDpy, NULL, devOut->layerList, n, &devOut->layerCount);
        memset(devOut->windowList, 0, (n*sizeof(struct NvGlDemoWindowDevice)));
    } else {
        NvGlDemoLog("peglGetOutputLayersEXT_fail[%d %d]\n",devOut->layerList,devOut->windowList);
        goto NvGlDemoCreateEglDevice_fail;
    }
...
static void NvGlDemoTermWinSurface(void)
{
    ...
    if (winDev->stream != EGL_NO_STREAM_KHR) {
...
static drmPlaneType GetDrmPlaneType(int drmFd, uint32_t objectID)
{
    uint32_t i;
    int j;
    int found = 0;
    uint64_t value = 0;
    drmPlaneType planeType = DRM_PLANE_TYPE_OVERLAY;
...
static bool NvGlDemoCreateDrmDevice( EGLint devIndx )
{
    ...
    devOut = &nvGlOutDevLst[devIndx];
    // If no outputs are available, just skip the rest
    if (devOut && !devOut->layerCount) {
        NvGlDemoLog("layer cnt is 0\n");
        return false;
    }
    ...
    for (i=0; i<devOut->layerCount; ++i) {
        EGLAttrib id = 0;
        if (peglQueryOutputLayerAttribEXT(devOut->eglDpy, devOut->layerList[i], EGL_DRM_CRTC_EXT, &id)) {
            for (j=0; j<nvGlDrmDev->res->count_crtcs; ++j) {
                if (nvGlDrmDev->res->crtcs[j] == (uint32_t)id) {
                    nvGlDrmDev->crtcInfo[j].layer = i;
                }
            }
        } else if (peglQueryOutputLayerAttribEXT(devOut->eglDpy, devOut->layerList[i], EGL_DRM_PLANE_EXT, &id)) {
            for (j=0; j<(int)nvGlDrmDev->planes->count_planes; ++j) {
                if (nvGlDrmDev->planes->planes[j] == id) {
                    nvGlDrmDev->planeInfo[j].layer = (uint32_t)i;
                }
            }
        } else {
            NvGlDemoLog("EGL_DRM_CRTC/PLANE_EXT fail\n");
            return false;
        }
    }

    // Verify all crtcs/planes have a layer
    for (i=0; i<nvGlDrmDev->res->count_crtcs; ++i) {
        if (nvGlDrmDev->crtcInfo[i].layer < 0) {
            return false;
        }
    }
    for (i=0; i<(int)nvGlDrmDev->planes->count_planes; ++i) {
        /* The planes array will include an extra plane of PRIMARY type
         * which was not exposed by the layerList that EGL created.
         * On one hand, we could make the lists line up between app and
         * EGL by supplying EGL_DRM_MASTER_FD_EXT of our UNIVERSAL_PLANES
         * enabled drm handle during the EGLDisplay creation.
         * However, this is not tested.
         *
         * So alternatively, the absence of PRIMARY type
         * planes from the EGL list is forgiven.
         */
        if (nvGlDrmDev->planeInfo[i].planeType == DRM_PLANE_TYPE_PRIMARY ||
                nvGlDrmDev->planeInfo[i].planeType == DRM_PLANE_TYPE_CURSOR) {
            continue;
        }

        if (nvGlDrmDev->planeInfo[i].layer < 0) {
            return false;
        }
    }
    return true;
...
// If display option is specified, but isn't supported, then exit.
    ...
    // But do not fail on the unsupported "desktopsize"
    // options (that are also a side effect of the "res" option).
    // We will ignore these on egldeviceEXT.

    if (demoOptions.desktopSize[0]) {
        NvGlDemoLog("Setting desktop size is not supported. Ignoring.\n");
    }
