...
NvGlDemoDisplayTerm(void)
{
    ...
    if (demoState.platform->display.ivi_application) {
        ...
        if (demoState.platform->display.has_ivi_controller) {
            ilmClient_destroy();
        } else
#endif
        {
...
static int create_ivi_surface(struct Window *window, struct Display *display)
{
    ...
    display->has_ivi_controller = 0;
    if (ilm_initWithNativedisplay(native_display) == ILM_SUCCESS) {
        display->has_ivi_controller = 1;
        ilm_destroy();
    }
    ...
    const char *appName)
{
    ...
    } else if (demoState.platform->display.ivi_application) {
