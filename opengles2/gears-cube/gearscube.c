...
cubeSceneRender(void)
{
    ...
    // For pbuffer bind method, bind gears surface to texture
    #ifdef METHOD_PBUFFER
    if (method == Method_PbufferBind)
        eglBindTexImage(demoState.display, gearsSurface, EGL_BACK_BUFFER);
    #endif
    ...
    // For pbuffer bind method, release gears surface from texture
    #ifdef METHOD_PBUFFER
    if (method == Method_PbufferBind)
        eglReleaseTexImage(demoState.display, gearsSurface, EGL_BACK_BUFFER);
    #endif
...
gearsMethodInit(void)
{
    ...
    switch (method) {
        ...
        #ifdef METHOD_PBUFFER
        case Method_PbufferBind:
            cfgAttrs[3] = EGL_TRUE;
            surfDesc = "pbuffers with EGL_BIND_TO_TEXTURE_RGB";
            // fall through
...
int main(int argc, char **argv)
{
    ...
    while (argc > 1) {
        ...
        if (NvGlDemoArgMatchStr(&argc, argv, 1, "-method",
            ...
            #endif // METHOD_PIXMAP
            #ifdef METHOD_PBUFFER
            if (!STRCMP(methodName, "pbufferbind"))
                method = Method_PbufferBind;
            else
        ...
        // Run time
            ...
            NvGlDemoArgExtract(&argc, argv, 1);
        }
    ...
    //   was due to bad command line arguments.
        ...
        NvGlDemoLog("Usage: gearscube [options] [runTime]\n"
