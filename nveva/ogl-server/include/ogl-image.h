...
#ifndef __OGL_IMAGE_H__
#define __OGL_IMAGE_H__

#include "ogl-types.h"

typedef struct {
	GLuint format;		/* image format, for example GL_RGB */
	GLuint type;		/* image type, for example GL_FLOAT */
	uvec2_t size;		/* width and height in pixels */
	GLuint scanline;	/* size of scanline in bytes */
	GLuint bytes;		/* size of image in bytes */
	GLvoid* data;		/* pointer to raw image data */
} ogl_image_t;
...
extern void oglImageDelete(ogl_image_t *img);

extern GLvoid* oglImageGenerate(ogl_image_t *img, GLuint format, GLuint type,
				GLuint width, GLuint height);

extern GLvoid *oglImageFromColorMap(ogl_image_t *img, GLuint format,
				    GLuint type, GLuint width, GLuint height,
				    GLvoid *ind, GLenum itype,
				    GLvoid *cmap, GLuint colors);

extern GLvoid* oglImageFromFile(ogl_image_t *img, const char *path);

#ifdef __cplusplus
	};
#endif

#endif	/* __OGL_IMAGE_H__ */
