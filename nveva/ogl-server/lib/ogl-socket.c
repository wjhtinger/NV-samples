...
int oglCreateSocket(void)
{
	return socket(AF_INET, SOCK_STREAM, 0);
}

int oglSockTCPDelay(int sock, int disable)
{
	return setsockopt(sock, IPPROTO_TCP, TCP_NODELAY,
			  (char *)&disable, sizeof(disable));
}

int oglBindSocket(int sock, unsigned port)
{
...
int oglBindToDev(int sock, const char *name)
{
...
int oglListenSocket(int sock)
{
	return listen(sock , 1);
}

int oglCreateAcceptSocket(int sock)
{
...
int oglConnectSocket(int sock, const char *ip, unsigned port)
{
	struct sockaddr_in server_addr;
	...
	return connect(sock, (struct sockaddr *)&server_addr,
		       sizeof(server_addr));
}

int oglSendStringSocket(int sock, char *buf)
{
...
int oglReceiveStringSocket(int sock, char *buf, unsigned len)
{
	int retval;
