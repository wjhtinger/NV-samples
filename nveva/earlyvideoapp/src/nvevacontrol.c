...
//------------------------------------------------------------------------------
//! \file nvevacontrol.c
//! \brief Eva video control example implementation
//------------------------------------------------------------------------------

#include "nvevacontrol.h"

NvResult EvaControlCameraSelect (ECamera_ID camID)
{
    EarlyVideoApp * pEarlyVideo;
    pEarlyVideo = GetEarlyVideoApp ();
    if (pEarlyVideo)
        return EVA_CameraSelect (pEarlyVideo, camID);

    return RESULT_NOT_READY;
}

NvResult EvaControlPictureControl (float Brightness, float Contrast, float Saturation, float Hue)
{
    EarlyVideoApp * pEarlyVideo;
    pEarlyVideo = GetEarlyVideoApp ();
    if (pEarlyVideo)
        return EVA_PictureControl (pEarlyVideo, Brightness, Contrast, Saturation, Hue);

    return RESULT_NOT_READY;
}

NvResult EvaControlWelcomeAnimationPlay ()
{
    EarlyVideoApp * pEarlyVideo;
    pEarlyVideo = GetEarlyVideoApp ();
    if (pEarlyVideo)
        return WelcomeAnimationPlay (pEarlyVideo);
